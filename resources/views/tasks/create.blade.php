    ...
    <x-slot:title>Create New Task</x-slot>
        ...
        <div class="mt-5 mb-5 border-3 px-10 py-5 rounded w-1/2 m-auto border-white">
            <form action="/tasks/store" method="post">
                ...
                <div class="mb-3"><label for="title">Title</label><br> <input class="mt-1 w-full border border-white rounded p-2" type="text" name="title" id="title">
                    @error('title')
                    <p class="mt-1 text-xs text-red-500">{{$message}}</p>
                    @enderror
                </div>
                <div><label for="description">Description</label><br><textarea rows="5" class="mt-1 w-full border border-white rounded p-2" name="description" id="description"></textarea>
                    @error('description')
